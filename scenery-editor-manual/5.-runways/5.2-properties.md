...
: Runway property changes don't take effect immediately. To make them reflect, simply place an object, like an Electrical Equipment Building, next to the runway and move it after changing the runway properties. It will make the runway update to the new settings. Don't forget to remove the object when done.
...
| Button | Function | Notes |
| ------ | -------- | ----- |
| Overrun | You can change the runway overrun area (in meters) | See 5.2.4 below for more details|
| DisplacedThreshold | You can change how much the runway threshold is dispalced (in meters) | See 5.2.4 below for more details|
...
> [^1] In case a runway identifier for an ILS end of a runway has been modified, open a Reviewer ticket in the IFAET Discord server, and notify them of the runway end identifier change. An app restart will be required after changing identifiers (Make sure to save before restarting). Taxiway network checker will not work until the database has been updated (typically just before an app update)
...
![Image 5.2.4.1 - Runway End Markings](_images/manual/scenery-editor/runway-end-markings.jpg)

The red dot shown in the image above is where the runway end point should be located when editing runways

Overrun - The portion of the runway marked with yellow chevons, not meant for use by aircraft (unless in case of an emergency)

Displaced Threshold - This is where the Threshold of the runway is further down the runway due to landing restrictions. The portion in front of it can be used for takeoffs. The runway end point should be located at the beginning of this area, then the threshold displaced with the correct property (as seen above)